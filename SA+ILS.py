...
def calculate_cost_balanced(colors, edges, penalty_weight=75, balance_weight=10, penalty_conflicts = 100):
    ...
    Retorna:
        - cost (float): Soma dos custos de conflitos, numero de conflitos, desbalanceamento e número de cores.
    """
...
def simulated_annealing(num_vertices, edges, colors, initial_temp=None, max_iter=5000):
    """
    Refinamento com Simulated Annealing utilizando Resfriamento Adaptativo e
    dependência do histórico de soluções, agora chamando o ILS para refinamento.
    ...
    for iteration in range(max_iter):
        ...
        perturbed_colors = diversified_perturbation(current_colors, num_vertices, edges, min(10, intensity), conflicts, balance_penalty, color_penalty)

        # Chamar o ILS para refinar a solução perturbada
        new_colors = ils_coloring(num_vertices, edges, perturbed_colors, max_iterations=100)
        ...
        # # Estratégia de resfriamento logarítmico
        ...
        # # Estratégia de resfriamento linear
...
def ils_coloring(num_vertices, edges, initial_solution, max_iterations):
    """
    Iterated Local Search (ILS) para busca local.
    ...
    for _ in range(max_iterations):
        ...
        perturbed_solution = diversified_perturbation(current_solution, num_vertices, edges, min(3, intensity), conflicts, balance_penalty, color_penalty)
...
def process_file(file_path, k, num_runs):
    """
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Result\\SA_ILS.txt")
