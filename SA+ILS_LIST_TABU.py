...
def calculate_cost_balanced(colors, edges, penalty_weight=150, balance_weight=10, penalty_conflicts = 100):
    """
    ...
    return conflicts + balance_penalty + color_penalty, conflicts_num, round(balance, 2), len(set(color_counts))
...
def simulated_annealing(num_vertices, edges, colors, initial_temp=None, max_iter=5000, tabu_size=50):
    """
    Refinamento com Simulated Annealing utilizando Resfriamento Adaptativo e Lista Tabu.
    ...
    Retorna:
        ...
        - best_cost (float): Custo da melhor solução encontrada.
    """
    current_colors = colors[:]
    current_cost, conflicts, balance_penalty, color_penalty = calculate_cost_balanced(current_colors, edges)
    ...
    for iteration in range(max_iter):
        ...
        # Aplicar perturbação na solução atual
        perturbed_colors = diversified_perturbation(
            current_colors, num_vertices, edges, min(10, intensity), conflicts, balance_penalty, color_penalty
        )

        # Adicionar solução à Lista Tabu
        if tuple(perturbed_colors) in tabu_list:
            continue
        if len(tabu_list) >= tabu_size:
            tabu_list.pop(0)
        tabu_list.append(tuple(perturbed_colors))

        # Chamar o ILS para refinar a solução perturbada
        new_colors = ils_coloring(num_vertices, edges, perturbed_colors, max_iterations=100, tabu_size=tabu_size)

        new_cost, conflicts, balance_penalty, color_penalty = calculate_cost_balanced(new_colors, edges)

        # Critério de aceitação do SA
        cost_diff = new_cost - current_cost
        acceptance_prob = math.exp(-cost_diff / max(temperature, 1e-10)) if cost_diff > 0 else 1.0
        ...
        if acceptance_prob > random.random():
            ...
            if new_cost < best_cost:
                ...
                intensity = 1  # Resetar a intensidade após melhoria
        else:
            intensity += 1  # Aumentar a intensidade se não aceitou a solução
    ...
    return best_colors, best_cost, conflicts, balance_penalty, color_penalty
...
def calculate_initial_temperature(num_vertices, edges, colors, iterations=50):
    ...
    for _ in range(iterations):
        new_colors = diversified_perturbation(current_colors, num_vertices, edges, intensity, conflicts, balance_penalty, color_penalty)
        new_cost, conflicts, balance_penalty, color_penalty = calculate_cost_balanced(new_colors, edges)
...
def diversified_perturbation(colors, num_vertices, edges, intensity, conflicts, balance_penalty, color_penalty):
    """
    ...
    # Ajustar intensidade relativa com base nos custos
    total_cost = conflicts + balance_penalty + color_penalty
    if total_cost == 0:
        return colors  # Se nenhum custo existe, não perturba
    
    # Pesos proporcionais aos custos
    conflict_weight = conflicts / total_cost
    balance_weight = balance_penalty / total_cost
    color_weight = color_penalty / total_cost

    # Escalar intensidade com base nos pesos
    conflict_intensity = int(intensity * conflict_weight) + 1
    balance_intensity = int(intensity * balance_weight) + 1
    color_intensity = int(intensity * color_weight) + 1
    
    new_colors = colors[:]    
    for perturbation_type in ["reduce_colors", "swap_adjacent", "balance_colors"]:
        if perturbation_type == "reduce_colors" and color_penalty > 0:
            # Reduzir cores apenas se não aumentar conflitos
            for vertex in random.sample(range(num_vertices), min(color_intensity, num_vertices)):
        ...
        elif perturbation_type == "swap_adjacent" and conflicts > 0:
            # Identificar arestas conflitantes
            conflicting_edges = [(u, v) for u, v in edges if new_colors[u - 1] == new_colors[v - 1]]
            
            # Iterar sobre uma amostra das arestas conflitantes, até o limite da intensidade
            for u, v in random.sample(conflicting_edges, min(conflict_intensity, len(conflicting_edges))):
        ...
        elif perturbation_type == "balance_colors" and balance_penalty > 0:
            # Balancear cores com maior intensidade para alto desbalanceamento
            color_counts = Counter(new_colors)
            if len(color_counts) > 1:
                for _ in range(balance_intensity):
...
def ils_coloring(num_vertices, edges, initial_solution, max_iterations, tabu_size=50):
    """
    Iterated Local Search (ILS) com Lista Tabu.
    
    Parâmetros:
        - num_vertices (int): Número de vértices do grafo.
        - edges (list): Lista de arestas do grafo.
        - initial_solution (list): Solução inicial a ser refinada.
        - max_iterations (int): Número máximo de iterações do ILS.
        - tabu_size (int): Tamanho da Lista Tabu.
        
    Retorna:
        - best_solution (list): Melhor solução encontrada pelo ILS.
    """
    current_solution = initial_solution[:]
    current_cost, conflicts, balance_penalty, color_penalty = calculate_cost_balanced(current_solution, edges)
    best_solution = current_solution[:]
    best_cost = current_cost
    intensity = 1

    tabu_list = []  # Lista Tabu para evitar revisitar soluções recentes

    for _ in range(max_iterations):
        # Perturbação diversificada
        perturbed_solution = diversified_perturbation(
            current_solution, num_vertices, edges, min(3, intensity), conflicts, balance_penalty, color_penalty
        )

        # Adicionar solução à Lista Tabu
        if tuple(perturbed_solution) in tabu_list:
            continue  # Ignorar soluções tabu
        if len(tabu_list) >= tabu_size:
            tabu_list.pop(0)  # Remover soluções mais antigas
        tabu_list.append(tuple(perturbed_solution))  # Adicionar nova solução

        perturbed_cost, conflicts, balance_penalty, color_penalty = calculate_cost_balanced(perturbed_solution, edges)

        # Verificar melhorias
        if perturbed_cost < best_cost:
            best_solution = perturbed_solution[:]
            best_cost = perturbed_cost
            intensity = 1  # Resetar intensidade após melhoria
        else:
            intensity += 1  # Aumentar intensidade se não houve melhoria
...
def process_file(file_path, k, num_runs=10):
    ...
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Result\\SA_ILS_LISTA_TABU.txt")
    with open(output_path, 'a', encoding='utf-8', newline='\n') as f:
        ...
        try:
            ...
            for run in range(num_runs):
                ...
                refined_sa_solution, refined_sa_cost, _, _, _ = simulated_annealing(num_vertices, edges, initial_random_colors)
                ...
                refined_sa_greedy_solution, refined_sa_greedy_cost, _, _, _ = simulated_annealing(num_vertices, edges, initial_greedy_colors)
